...
def test_list():
    ...
    test_slides_list = [
        "https://www.wikidata.org/?uselang=de",  # from wikipedia
        "https://commons.wikimedia.org/?uselang=de",  # from wikipedia
        "https://de.wikipedia.org",  # from wikipedia
        (configs.wikipedia_list_cache / "2000px-Strategy_Graphic_-_High_level.svg.png")
        .resolve()
        .as_uri(),  # from wikipedia
        ...
        (configs.local_lists_cache / "WikiMUC_Garamond2.jpg")
        .resolve()
        .as_uri(),  # form ./test/test_list.txt
        (configs.working_directory / "Angertorstr._3_mit_WikiMUC_bearbeitet.jpg")
        .resolve()
        .as_uri(),  # form ./test/*
    ]
...
def test_cache_renewal():
    ...
    if shifted_timestamp != slideshow.timestamp:
        print(
            "\033[31m" + "ERROR: The slides list got updated prematurely!" + "\033[0m"
        )
    ...
    if shifted_timestamp == slideshow.timestamp:
        print(
            "\033[31m"
            + "ERROR: The slides list did not get updated after the cache_lifetime!"
            + "\033[0m"
        )
        print("cache_lifetime: ", configs.cache_lifetime)
        print("Tested time shift: ", timeshift)
    ...
    print(
        "\033[32m"
        + "Huzzah! slideshow cache update is working as expected!"
        + "\033[0m"
    )
    return 0
...
def test_errors():
    ...
    try:
        ...
        print(
            "\033[31m"
            + "ERROR: Couldn’t initialize the Telescreen with an invalid working directory: "
            + str(exc)
            + "\033[0m"
        )
        return 1
    ...
    print("Testing invalid image URL.")
    invalid_list = configs.working_directory / "ghost.txt"
    invalid_list.open("w").write("https://de.wikipedia.org/wtf/please_dont_exist.jpg")
    ...
    try:
        ...
        print(
            "\033[31m"
            + "ERROR: Couldn’t initialize the Telescreen with an invalid image URL: "
            + str(exc)
            + "\033[0m"
        )
        return 1
    ...
    try:
        ...
        print(
            "\033[31m"
            + "ERROR: Couldn’t initialize the Telescreen with no revision from the whitelisted users: "
            + str(exc)
            + "\033[0m"
        )
        return 1
    ...
    try:
        ...
        print(
            "\033[31m"
            + "ERROR: Couldn’t initialize the Telescreen with an invalid Wikipedia page: "
            + str(exc)
            + "\033[0m"
        )
        return 1
    ...
    print(
        "\033[32m" + "Whoopee! All the tested exceptions are being handled!" + "\033[0m"
    )
    return 0
